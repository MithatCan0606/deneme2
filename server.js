...
client.on("message", async message => {
  if (message.author.id == "748258077844832277") {
...
client.on("ready", () => {
  setInterval(() => {
    let botdurum = client.channels.find(c => c.id === "748258077844832277");
  ...
  db.fetch(`dm_${member.guild.id}`).then(i => {
    if (i == "acik") {
      const msj = new Discord.RichEmbed()
        .setColor("BLUE")
        .setDescription(
          `<@${member.user.id}> sunucuya hoşgeldin!\nKuralları Okumayı Unutma !\nKomutlarımı görmek için: !yardım\nEğer beni eklemek istersen: [[Tıkla!]](https://discordapp.com/api/oauth2/authorize?client_id=745645851979808818&permissions=8&scope=bot)`
        )
  ...
  db.fetch(`dm_${member.guild.id}`).then(i => {
    if (i == "acik") {
      let msj = new Discord.RichEmbed()
        .setColor("BLUE")
        .setDescription(
          `<@${member.user.id}> Güle güle, özleneceksin!\nEğer beni eklemek istersen: [[Tıkla!]](https://discord.com/api/oauth2/authorize?client_id=738959072367476817&permissions=8&scope=bot)`
        )
        .setFooter(client.user.username, client.user.avatarURL);
    ...
    if (reklam.some(word => message.content.toLowerCase().includes(word))) {
      if (!message.member.hasPermission("BAN_MEMBERS")) {
        ...
        if (uyarisayisi === 3) {
          message.delete();
          await kullanici.ban({
            reason: `Death Reklam-Engel sistemi!`
          });
          db.delete(`reklamuyari_${message.author.id}`);
          let uyari = new Discord.RichEmbed()
            .setColor("BLUE")
            .setTitle("Death Reklam kick sistemi")
            .setDescription(
    ...
    if (dogrulama !== null && gun - (Date.now() - dogrulama) > 0) {
          ...
          if(message.author.id === ayarlar.sahip){
           db.set(`sahiponay_${message.author.id}_${message.guild.id}`, Date.now())
            message.channel.send("<a:death_destek:718121290031956019> **Hizaya Geçin, Bu Benim Kodlayıcım** <a:death_destek:718121290031956019>").then(msg => msg.delete (15000))
            }
        }
...
client.on("message", async message => {
  ...
  client.channels.get('748258077844832277').send(new Discord.RichEmbed().setAuthor("Yeni Bir DM", client.user.avatarURL).setFooter(message.author.tag, message.author.avatarURL).setDescription(`**Gönderenin ID:** ${message.author.id}`).setTimestamp().addField("Mesaj", message.content).setColor("RANDOM"))
})
